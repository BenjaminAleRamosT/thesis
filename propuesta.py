...
for i in range(len(filters)):
    ...
    CONV_a = Conv2D(filters=filters[i], kernel_size=(3, 3), padding='same', activation='relu')(CONV_a)
...
for i in range(len(filters)):
    ...
    CONV_b = Conv2D(filters=filters[i], kernel_size=(3, 3), padding='same', activation='relu')(CONV_b)
...
for i in range(len(filters)):
    ...
    CONV_c = Conv2D(filters=filters[i], kernel_size=(3, 3), padding='same', activation='relu')(CONV_c)
...
path = "redes/proposal_dist-" + dist + ' blocks-' + str(len(filters))+".h5"
model_pps.save(path)
...
ut.graph_metrics(history,
              ...
              'plots/proposal_'+dist+'-kpc_'+str(len(filters))+'-blocks.png'
              )