...
#define model
model_pps2 = Model(inputs=inputs,outputs=[out], name = "model_pps2")


   
model_pps2.compile(optimizer=Adam(learning_rate=0.001), loss='binary_crossentropy', metrics=['accuracy',tf.keras.metrics.FalsePositives()])
model_pps2.summary()
#keras.utils.plot_model(model_pps2, show_shapes=True)
...
history = model_pps2.fit(my_training_batch_generator,steps_per_epoch=int( len(X_train_filenames) // batch_size),
                            validation_data=my_validation_batch_generator,
                            validation_steps=int( len(X_val_filenames) // batch_size),
                            epochs=100,
                            verbose=1)

path = "redes/proposal2_dist-" + dist+".h5"
model_pps2.save(path)
