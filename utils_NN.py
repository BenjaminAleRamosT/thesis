...
def name_store(directory='data/timeFrecuency'):
    ...
    for i in range(len(dist)):
        signal_names = glob.glob(
            directory+'/s11.2--LS220_'+dist[i]+'kpc_sim*.txt')
...
#name_store('data/2000 datafull')

def transform_data(batch_x , trns_indx, fmax = 2048):
    trns_funct = [pr.calcular_stft, pr.wt_morlet,
                      pr.dwt_pywt, pr.calcular_cwt, pr.calcular_fcwt, pr.mel]
