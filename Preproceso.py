    ...
    for filename in os.listdir(directory):
        ...
        if os.path.isfile(f):
            ...
            for i in range(4):
                ...
                outfile = outdir + '/' + \
                    Path(filename).stem + '_' + str(i) + '.txt'
                np.savetxt(outfile, data_shifted, delimiter=' ')


def calcular_stft(data, sampling_rate=16384, graph=False, fmax=8192,  window_length=1024, num_segments=144, step_ratio=4):
    """
    ...
    f, t, Zxx = stft(data, fs=sampling_rate, window='hann',
                     nperseg=window_length, noverlap=window_step, nfft=window_length)
    ...
    if graph:
        ...
        plt.pcolormesh(t, f, np.abs(Zxx), cmap='twilight',
                       vmax=abs(Zxx).max(), vmin=-abs(Zxx).max())
...
def wt_morlet(data, sampling_rate=16384, graph=False, fmax=8192):
    ...
    if graph:
        plt.pcolormesh(np.abs(tfr_morlet), cmap='twilight', vmax=abs(
            tfr_morlet).max(), vmin=-abs(tfr_morlet).max())
...
def dwt_pywt(data, sampling_rate=16384, graph=False, fmax=8192):
    ...
    """
    freqs = np.arange(
        1, fmax, 16)  # Define the frequencies to analyze in the DWT
    ...
    if graph:
        coeff = coeff[coeff.shape[0]//2:]
        plt.pcolormesh(np.abs(coeff), cmap='twilight',
...
def mel(data, sampling_rate=16384, graph=False, fmax=8192, n_fft=2048, hop_length=64, n_mels=40):
    """
    ...
    if graph:
        ...
        librosa.display.specshow(S, sr=sampling_rate, hop_length=hop_length, x_axis='time',
                                 y_axis='mel', cmap='twilight', vmax=abs(S).max(), vmin=-abs(S).max())
...
# f = 'data/samples/s15.0--SFHo_10kpc_sim2.txt'
# data = np.loadtxt(f, delimiter=" ")[:, 1]
# print(mel(data, graph = True,fmax=2048 ,n_fft= 512,hop_length=32, n_mels=20).shape,
# mel(data, graph = True,fmax=2048 ,n_fft= 1024,hop_length=64, n_mels=40).shape,
# mel(data, graph = True,fmax=2048 ,n_fft= 2048,hop_length=256, n_mels=60).shape)
...
def Hilbert_Huang(data, sampling_rate=16384, graph=False, fmax=8192):
    ...
    return imfs
# f = 'data/2000 datafull/s11.2--LS220_0.1kpc_sim2.txt'
# data = np.loadtxt(f, delimiter=" ")[:,1]
# Hilbert_Huang(data)
...
def prepro_dir(directory, outdir, trns_indx=0, graph=False, white=False, pad=0, fmax=8192, verbose=0):
    ...
    trns_funct = [calcular_stft, wt_morlet,
                  dwt_pywt, calcular_cwt, calcular_fcwt, mel]
    ...
    if verbose:
        ...
        dataTransformed = trns_funct[trns_indx](
            serie_de_tiempo, graph=False, fmax=fmax)
